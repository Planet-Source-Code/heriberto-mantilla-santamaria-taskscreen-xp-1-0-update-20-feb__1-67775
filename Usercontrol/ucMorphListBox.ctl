...
Begin VB.UserControl MorphListBox 
   ...
   EndProperty
   MouseIcon       =   "ucMorphListBox.ctx":0000
   MousePointer    =   99  'Custom
   ScaleHeight     =   122
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   81
   ToolboxBitmap   =   "ucMorphListBox.ctx":0152
End
...
'  events.
Public Event Changed(Index As Long)
Public Event Click()
...
Private AltKeyDown                       As Boolean       ' global "alt key being pressed" flag.
...
'  it.  Ignore this if drag and drop operation is enabled.
   ...
   If (MouseOverIndex(MouseY) <= (m_ListCount - 1)) And (MouseOverIndex(MouseY) >= 0) Then
       If AltKeyDown = False And MouseOverIndex(MouseY) <> LastSelectedItem Then
           RaiseEvent Changed(MouseOverIndex(MouseY))
       Else
           RaiseEvent Changed(LastSelectedItem)
       End If
...
'     determine Shift and Ctrl key status.
      ...
      AltKeyDown = (Shift And vbAltMask) > 0
...
'*************************************************************************
   ...
   If m_Enabled Then
'     determine shift, alt and Ctrl key status.
      ShiftKeyDown = (Shift And vbShiftMask) > 0
      CtrlKeyDown = (Shift And vbCtrlMask) > 0
      AltKeyDown = (Shift And vbAltMask) > 0
...
'  display the text using DrawText api.
   Call DrawText(UserControl.hDC, ListArray(Index), -1, r, DT_LEFT Or DT_NOPREFIX Or DT_SINGLELINE Or DT_WORD_ELLIPSIS)
...
Public Property Get LastItem() As Long
   LastItem = LastSelectedItem
End Property
